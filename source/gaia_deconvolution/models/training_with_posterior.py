...
class GaiaModel:
    ...
    def train(self, training_sample, validation_sample, args, show_plots=True, save_best_state=True):      
        ...
        train = Train_Epoch(self.models, args)
        test = Evaluate_Epoch(self.models, args)
        ...
        for model in self.keys:
            ...
            optimizer[model] = torch.optim.AdamW(self.models[model].parameters(), lr=args.lr)  
            scheduler[model] = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer[model], args.max_epochs)
            print('INFO: number of training parameters: {}'.format(sum(p.numel() for p in self.models[model].parameters())))
        ...
        for epoch in tqdm(range(args.max_epochs), desc="epochs"):
            ...
            for model in self.keys(): scheduler[model].step() 
