...
params = argparse.ArgumentParser(description='arguments for the deconvolution model: the flow learns the clean data p(v)')
...
params_qv = copy_parser(params, 
                         description='arguments for the approx posterior model q(v|w) for VI',
                         modifications={
...
params_pw = copy_parser(params, 
                        description='arguments for pre-trining model: the flow learns the noisy data p(w)',
                        modifications={
...
if __name__ == '__main__':
    ...
    if args.flow == 'MAF': p = masked_autoregressive_flow(args)
    elif args.flow == 'coupling':  p = coupling_flow(args)
    q =  inverse_autoregressive_flow(args)

    flow['prior'] = p.to(args.device)      # p(v) truth density
    flow['posterior'] = q.to(args.device)  # q(v|w) approximate posterior 2006.09396
